...
def setup_recon_ws(ws, fit_cfg, logdir="logs", name=None):
...
def update_s2d_track_identification(
    ...
    if not use_motion_mask:
        ...
        motion_mask_dir = osp.join(s2d.ws, "motion_mask")
        ...
        s2d.register_gradfree_buffer("motion_mask_list", motion_mask_list)

        with torch.no_grad():
            motion_list = query_buffers_by_track(
                s2d.motion_mask_list.clone()[..., None], s2d.track, s2d.track_mask
            )
            motion_list = motion_list.squeeze(-1).cpu()
        
        motion_list = torch.maximum(epi_error_list, motion_list)  # exp use this
        epi_track_static_selection, epi_track_dynamic_selection = identify_tracks(
            motion_list, epi_th, static_cnt=1, dynamic_cnt=dyn_id_cnt
        )
