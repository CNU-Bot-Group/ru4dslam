...
class StaticGaussian(nn.Module):
    ...
    @classmethod
    def load_from_ply(cls, path, scale_nw, max_sh_degree=0):
        ...
        f_dc0 = plydata.elements[0]["f_dc_0"]
        f_dc1 = plydata.elements[0]["f_dc_1"]
        f_dc2 = plydata.elements[0]["f_dc_2"]
        ...
        features_extra = features_extra.reshape(
            (features_extra.shape[0], 3, int(len(extra_f_names)/3))
        )
        current_length = int(len(extra_f_names)/3)
        expected_length = (max_sh_degree + 1) ** 2 - 1

        if current_length < expected_length:
            padding_length = expected_length - current_length
            features_extra = np.pad(
                features_extra,
                pad_width=((0, 0), (0, 0), (0, padding_length)),  # 在列方向补 0
                mode='constant',
                constant_values=0
            )
        ...
        features_dc = torch.as_tensor(features_dc).squeeze(-1).float()

        xyz = xyz * scale_nw
        # scales = scales * scale_nw * scale_nw

        init_mean = xyz
        init_rgb = features_dc
        init_s = scales
        init_s = torch.exp(torch.as_tensor(init_s).float())
        max_sph_order = max_sh_degree
        model = cls(
            init_mean=init_mean,
            init_rgb=SH2RGB(init_rgb),
            init_s=init_s.float(),
            max_sph_order=max_sph_order,
            max_scale=1,
        )

        model.max_scale.copy_(torch.as_tensor(1).squeeze())

        model.min_scale.copy_(torch.as_tensor(0.0).squeeze())

        model.max_sph_order.copy_(torch.as_tensor(max_sph_order).squeeze())

        # model.group_id.copy_(torch.as_tensor(data['group_id']))

        # model.xyz_gradient_accum.copy_(torch.as_tensor(data['xyz_gradient_accum']))

        # model.xyz_gradient_denom.copy_(torch.as_tensor(data['xyz_gradient_denom']))

        # model.max_radii2D.copy_(torch.as_tensor(data['max_radii2D']))

        model._xyz.data.copy_(torch.as_tensor(xyz).float())

        model._rotation.data.copy_(torch.as_tensor(rots).float())

        # 注意：_scaling需要通过s_inv_act转换
        model._scaling.data.copy_(model.s_inv_act(init_s * scale_nw))

        # 注意：_opacity需要通过o_inv_act转换
        opacity = model.o_act(torch.as_tensor(opacities).float())
        model._opacity.data.copy_(model.o_inv_act(opacity))

        model._features_dc.data.copy_(features_dc)

        model._features_rest.data.copy_(torch.as_tensor(features_extra).float().view(model._features_rest.shape))
    ...
    def forward(self, active_sph_order=None):
        ...
        return xyz, frame, s, o, sph #, self._rotation
