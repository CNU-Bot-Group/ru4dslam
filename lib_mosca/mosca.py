...
class MoSca(nn.Module):
    def __init__(
        ...
        #     self.t_center = nn.Parameter(torch.as_tensor(t_center).to(init_device))
        #     self.t_scale = nn.Parameter(torch.as_tensor(t_scale).to(init_device))
        # else:
        #     self.t_center = nn.Parameter(torch.ones((1, self.M, 1), device=init_device)*0.83)
        #     self.t_scale = nn.Parameter(torch.ones((1, self.M, 1), device=init_device))

        if t_center is not None:
            self.t_center = nn.Parameter(torch.as_tensor(t_center).to(init_device))
            self.t_scale = nn.Parameter(torch.as_tensor(t_scale).to(init_device))
        else:
            # self.t_center = nn.Parameter(torch.zeros((len(self._t_list), self.M, 1)).to(init_device))
            # self.t_scale = nn.Parameter(torch.zeros((len(self._t_list), self.M, 1)).to(init_device))

            self.t_center = nn.Parameter(torch.ones((len(self._t_list), self.M, 1)).to(init_device)*2)
            self.t_scale = nn.Parameter(torch.ones((len(self._t_list), self.M, 1)).to(init_device)*2)
    ...
    def update_topology(
        ...
        if len(unique_group_id) == 1 or not self.break_topo_between_group:
            self._D_topo = _compute_curve_topo_dist_( # N N 计算所有动态点，两两之间距离，在所有T中最远距离
                curve_xyz=self._node_xyz,
        ...
        topo_dist, topo_ind = __compute_topo_ind_from_dist__( # 所有动态点两两之间最大距离的前15近的距离
            self._D_topo, self.skinning_k - 1
        )

        self_ind = torch.arange(self.M).to(self.device)
        topo_ind = torch.cat([self_ind[:, None], topo_ind], dim=1)#所有动态点两两之间最大距离的前16近的距离，而最近的 是自己。
        topo_th = self.topo_th_ratio * self.spatial_unit
    ...
    def update_multilevel_arap_topo(self, verbose=False):
        if not self.mlevel_arap_flag:
            return
        topo_th = self.topo_th_ratio * self.spatial_unit
        self.multilevel_arap_edge_list, self.multilevel_arap_dist_list = ( # 上一层的点连接这一层的哪个点，以及距离 
    ...
    def warp(
        ...
        # t_center_ind = (self.t_center[0].squeeze(-1))[sk_ind]
        # t_center = (t_center_ind * sk_w).sum(dim=-1).unsqueeze(-1)
        # t_scale = ((self.t_scale[0].squeeze(-1))[sk_ind] * sk_w).sum(dim=-1).unsqueeze(-1)
        
        # if o is not None:
        #     weight = torch.exp(
        #         -1 * 
        #         (
        #             (target_tid.float().unsqueeze(-1)/len(self._t_list) - torch.exp(-t_center**2))
        #             /
        #             (torch.exp(t_scale))
        #         )
        #         **2
        ...
        #     return ret_xyz, ret_dir, ret_o

        # else:
        #     return ret_xyz, ret_dir, None
        
        t_center_ind = (self.t_center[target_tid[0]].squeeze(-1))[sk_ind]
        t_center = (t_center_ind * sk_w).sum(dim=-1).unsqueeze(-1)
        t_scale = ((self.t_scale[target_tid[0]].squeeze(-1))[sk_ind] * sk_w).sum(dim=-1).unsqueeze(-1)

        # t_scale_np = t_scale.detach().cpu().numpy()
        # sk_np = sk_w.detach().cpu().numpy()
        if o is not None:
            # ret_o = o * (1 - torch.sigmoid(t_scale))
            ret_o = o * torch.sigmoid(t_scale)
            # loss = ret_o.mean()
            # if target_tid[0] == 32:
            #     loss.backward()
            #     tttt = torch.count_nonzero(self.t_scale.grad!=0.0)
            #     if tttt > 0:
            #         print(f"t_scale grad more than 0.0: {tttt.item()}")
            return ret_xyz, ret_dir, ret_o
    ...
    def resample_node(self, resample_factor=1.0, use_mask=False, resample_ind=None):
        ...
        # new_t_center = self.t_center[resample_ind]
        # new_t_scale = self.t_scale[resample_ind]
        new_t_center = self.t_center[:, resample_ind]
        new_t_scale = self.t_scale[:, resample_ind]
        with torch.no_grad():
            ...
            self.t_center = nn.Parameter(new_t_center)
            self.t_scale = nn.Parameter(new_t_scale)
            if len(self._node_grouping.unique()) < len(self.unique_grouping):
                logging.warning(
    ...
    def append_nodes_traj(
        ...
        #     "node_opa_t_center": torch.zeros(new_node_xyz_traj.shape[1], device=self.device),
        #     "node_opa_t_scale": torch.zeros(new_node_xyz_traj.shape[1], device=self.device),
        # }
        # spec = {"node_xyz": 1, "node_rotation": 1, "node_opa_t_center": 0, "node_opa_t_scale": 0}
        d = {
            "node_xyz": new_node_xyz_traj,
            "node_rotation": new_node_q_traj,
            "node_sigma": new_node_sigma_logit,
            "node_opa_t_center": torch.zeros((new_node_xyz_traj.shape[0],new_node_xyz_traj.shape[1], 1), device=self.device),
            "node_opa_t_scale": torch.zeros((new_node_xyz_traj.shape[0], new_node_xyz_traj.shape[1], 1), device=self.device),
            # "node_opa_t_center": torch.zeros((1, new_node_xyz_traj.shape[1], 1), device=self.device),
            # "node_opa_t_scale": torch.zeros((1, new_node_xyz_traj.shape[1], 1), device=self.device),
        }
        spec = {"node_xyz": 1, "node_rotation": 1, "node_opa_t_center": 1, "node_opa_t_scale": 1}
        ...
        self.t_center = optimizable_tensors["node_opa_t_center"]
        self.t_scale = optimizable_tensors["node_opa_t_scale"]
    ...
    def remove_nodes(self, optimizer, node_prune_mask):
        ...
        spec = ["node_xyz", "node_rotation", "node_sigma", "node_opa_t_center", "node_opa_t_scale"]
    ...
    def get_optimizable_list(
        ...
        if lr_tc is not None: # 0.01
            ret.append(
                {
                    "params": [self.t_center],
                    "lr": lr_tc,
                    "name": "node_opa_t_center",
                }
            )
        if lr_ts is not None: # 0.01
            ret.append(
                {
                    "params": [self.t_scale],
                    "lr": lr_ts,
                    "name": "node_opa_t_scale",
                }
            )
        return ret
    ...
    def __compute_R_from_xyz__(self):
        ...
        for new_tid in tqdm(range(1, self.T)):
            ...
            src_p = src_xyz[nn_ind] - src_xyz[:, None] # 当前时刻，每个点的前16近点与当前每个点的位置差
...
def resample_curve(D, sample_margin, mask=None, verbose=False):
    ...
    for ind in rank_inds[1:]: 
        if D[ind][sampled_inds].min() > sample_margin: # 如果新的点与已采样的点之间的最小距离大于采样阈值，则将其加入采样集合，即，令采样点两两之间的距离都大于采样阈值
...
def q2R(q):
    ...
    # # assert torch.all(nq > 1e-6)
    # norm = torch.norm(q, dim=-1, keepdim=True)
    # # 防止除以零
    # nq = torch.where(norm < 1e-6, 1, norm)
    # R = quaternion_to_matrix(torch.clamp(nq, min = 1))
...
def get_local_coord(
    ...
    nn_xyz = xyz[:, topo_ind, :] # 连接的16或8个topo点
    nn_R_wi = R_wi[:, topo_ind, :]
    if self_ind is None:
        self_xyz = xyz[:, :, None]  # T,N,1,3
    else:
        assert self_ind.shape == topo_ind.shape
        self_xyz = xyz[:, self_ind, :] # 中心节点
    if detach_self:
        self_xyz = self_xyz.detach()
    if detach_nn:
        nn_xyz = nn_xyz.detach()
    local_coord = torch.einsum(
        "tnkji,tnkj->tnki", nn_R_wi, self_xyz - nn_xyz # ji,j->i表示R.inv*x，ij,j->i表示R*x
    )  # T,N,K,3

    # t = torch.einsum(
    #     "tnkji,tnkj->tnki", nn_R_wi, self_xyz - nn_xyz
    # ).detach().cpu().numpy()  # T,N,K,3
    # t_inv = torch.einsum(
    #     "tnkji,tnkj->tnki", nn_R_wi.transpose(-1, -2), self_xyz - nn_xyz
    # ).detach().cpu().numpy()  # T,N,K,3
