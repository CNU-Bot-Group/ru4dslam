...
class DynSCFGaussian(nn.Module):
    ...
    def forward(self, t: int, active_sph_order=None, nn_fusion=None):
        ...
        #     -1 * 
        #     (
        #         (float(t)/len(self.t_center) - self.t_center[t])
        #         /
        #         (torch.exp(self.t_scale[t]))
        #     )
        #     **2
        #     )
        ...
        if self.fast_inference_flag:
            ...
            mu_live, fr_live, o_live = self.scf.warp(
                ...
                o=o if self.is_opa_control else None,
            )
        if o_live is None:
            o_live = o
        ...
        # loss = o_live.mean()
        # loss.backward()
        # return mu_live, fr_live, s, o, sph # , matrix_to_quaternion(fr_live)
        return mu_live, fr_live, s, o_live, sph # , matrix_to_quaternion(fr_live)
    ...
    def gradient_based_node_densification(
        ...
        grad_percentile = torch.quantile(grad, 0.9)

        print(f"grad thre: {gradient_th:.3f}, 90% percentile: {grad_percentile:.3f}")
        # top_10_percent_grads = grad[grad > grad_percentile]
...
def q2R(q):
    ...
    # if (norm < eps).any():
    #     print("Warning: Quaternion norm is very small!")
    # nq = q / (norm + eps)
