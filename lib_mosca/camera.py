...
class MonocularCameras(nn.Module):
    def __init__(
        ...
        self.uncer_weight = torch.nn.Parameter(torch.empty((self.T, default_H, default_W)), requires_grad=False)

        self.scale_nw = torch.nn.Parameter(torch.tensor(0.0), requires_grad=False)

        self.opt_rot_start = nn.Parameter(torch.tensor([1.0, 0.0, 0.0, 0.0]), requires_grad=True)
        self.opt_trans_start = nn.Parameter(torch.tensor([0.0, 0.0, 0.0]), requires_grad=True)
        self.opt_rot_end = nn.Parameter(torch.tensor([1.0, 0.0, 0.0, 0.0]), requires_grad=True)
        self.opt_trans_end = nn.Parameter(torch.tensor([0.0, 0.0, 0.0]), requires_grad=True)
    ...
    def update_poses(self, ind):
        ...
        param_cam_q, param_cam_t = self.__get_init_qt__(new_w2c.unsqueeze(0))
