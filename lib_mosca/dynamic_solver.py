...
def line_segment_init(track_mask, point_ref):
    ...
    for t in tqdm(range(T)):
        to_fill_mask = ~working_track_mask[t] # 当前要填充的
        ...
        if t == T - 1:  # if right end, use the previous one
            ...
            to_fill_valid_curve = working_track_mask[t + 1 :, to_fill_mask]  # T,M # 后面帧的有效点mask
            # find the left most True slot
            to_fill_valid_curve = ( # 近的点有较大的权重
                to_fill_valid_curve.float() * inverse_muti[t + 1 :, None]
            )
            max_value, max_ind = to_fill_valid_curve.max(dim=0) # 后面最近的点和最近的索引
            # for no right mask case, use the left
            select_from = working_point_ref[t + 1 :, to_fill_mask] # 后面帧的有效点云
            valid_right_end = torch.gather( # 获取后面最近的点云
            ...
            if t == 0:
                ...
                value = valid_right_end # 首帧未观测到，则认为动态点未动，填最初观测到的点
            else:
                # must have a left end
                value = working_point_ref[t - 1, to_fill_mask].clone() # 上一帧的值
                valid_left_end = value[max_value > 0] # 需要有后面观测到的帧，获得后面最近帧的点云
                delta_t = (#前后时间间隔
                    max_ind[max_value > 0] + 2
                )  # left valid, current, [0] in the max_ind
                delta_x = valid_right_end - valid_left_end # 前后帧点云位移
                inc = 1.0 * delta_x / delta_t[:, None] # 上一帧到当前帧位移
                value[max_value > 0] = valid_left_end + inc # 填补的位移
...
def get_dynamic_curves(
    ...
    if track.shape[-1] == 3:
        ...
        # spa tracker model
        # manually homo list
        homo_list = __int2homo_coord__(track[..., :2], s2d.H, s2d.W) # 所有track
        dep_list = track[..., -1]
        ...
        gathered_homo_list, gathered_dep_list, rgb_list = prepare_track_buffers( # 所有可见track点 动态
        ...
        if refilter_2d_track_flag and not return_all_curves:
            ...
            if refilter_unconsistent:
                ...
                print(f"spatracker consistency {(spatracker_consistent_mask).sum():d}")
                print(f"track mask: {track_mask.sum():d}")
                logging.info(
            ...
            if not refilter_2d_track_only_mask:
                ...
                max_num = 15000 
                if n > max_num:
                    selected_indices = torch.randperm(n)[:max_num]
                    
                    track_mask = track_mask[:, selected_indices]
                    track = track[:, selected_indices]
...
def geometry_scf_init(
    ...
    for step in tqdm(range(total_steps)):
        ...
        if not TEST_TRAINING_TIME_MODE and (step % viz_interval == 0 or step == total_steps - 1) and viz_interval > 0:
            viz_frame = viz_curve(
        ...
        if not TEST_TRAINING_TIME_MODE and viz_debug_interval > 0 and step % viz_debug_interval == 0:
