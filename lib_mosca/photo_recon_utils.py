...
from gs_utils.gs_optim_helper import get_expon_lr_func
...
def fetch_leaves_in_world_frame(
    ...
    for t in tqdm(t_list):
        ...
        if len(mu) > 0:
            dist = distCUDA2(mu.float().cuda())
            dist2 = torch.clamp_min(dist, 0.0000001)
            scale = torch.sqrt(dist2)[..., None].repeat(1, 3)
        else:
...
def error_grow_dyn_model(
    ...
    if just_dyn_region:
        ...
        grow_fg_masks = grow_fg_masks * s2d.dep_mask
    # viz
    imageio.mimsave(
