...
# cwd = os.getcwd()
# sys.path.append(os.path.join(cwd, '../MoSca'))
...
from ru4d_viz import viz_main, viz_list_of_colored_points_in_cam_frame
from ru4d_evaluate import test_tum_cam, test_sintel_cam, test_main, test_pck, test_fps
...
from lib_mosca.get_nodes import get_all_nodes
from lib_mosca.dynamic_gs import DynSCFGaussian

from colorama import Fore,Style
from time import gmtime, strftime

def get_static_render_error_mask(s2d, log_path, render_error_th, open_ksize=-1, TEST_TRAINING_TIME_MODE=False):
    assert osp.exists(
...
def photometric_warmup(ws, log_path, fit_cfg):
    ...
    s2d = (
        Saved2D(ws)
        .load_epi()
        .load_dep(DEPTH_DIR, DEPTH_BOUNDARY_TH, npz_path=npz_path)
...
def scaffold_reconstruct(ws, log_path, fit_cfg):
    ...
    s2d = update_s2d_track_identification( 
        ...
        use_motion_mask=getattr(fit_cfg, "use_motion_mask", False),
    )
    np.savez(
    ...
    if not TEST_TRAINING_TIME_MODE and s2d.has_epi:
    ...
    curve_xyz, curve_mask, curve_rgb, curve_filter_mask = get_dynamic_curves(
        ...
        * get_dynamic_curves_filter_factor,
        ...
        refilter_unconsistent=getattr(fit_cfg, "get_curve_refilter_unconsistent", True),
    )
    ...
    if getattr(fit_cfg, "geo_mosca_steps", 1500) > 0:
        scaffold = geometry_scf_init(
            ...
            viz_debug_interval=getattr(fit_cfg, "geo_ru4d_viz_debug_interval", -1),
            viz_interval=getattr(fit_cfg, "geo_ru4d_viz_interval", -1),
            viz_node_rgb=node_rgb,
            viz_level_flag=getattr(fit_cfg, "geo_ru4d_viz_level_flag", True),
            TEST_TRAINING_TIME_MODE=TEST_TRAINING_TIME_MODE,
        )
    if not TEST_TRAINING_TIME_MODE:
...
def photometric_reconstruct(ws, log_path, fit_cfg):
    ...
    photo_solver.identify_fg_mask_by_nearest_curve(
        s2d, cams, "gs_model_construct_fg_mask.mp4" if not TEST_TRAINING_TIME_MODE else None
    )
    if GS_BACKEND == "gof":
        photo_solver.compute_normals_for_s2d(
            s2d,
    ...
    ply_file_name = fit_cfg["data"]["output"] + "/" + fit_cfg["scene"] + "/final_gs.ply"

    if osp.exists(osp.join(log_path, f"photometric_s_model_{GS_BACKEND.lower()}_o.pth")):
        s_model = StaticGaussian.load_from_ckpt(
            torch.load(
                osp.join(log_path, f"photometric_s_model_{GS_BACKEND.lower()}_o.pth")
            ),
            device=device,
        ).to(device)
    else:
        if osp.exists(ply_file_name) and getattr(fit_cfg, "use_ru4d_ply", False):
            logging.info(f"Load static model from {ply_file_name}")
            s_model = StaticGaussian.load_from_ply(ply_file_name, s2d.scale_nw, max_sh_degree=getattr(fit_cfg, "gs_max_sph_order", 0)).to(device)
        elif osp.exists(s_model_warmup_path):
    ...
    if osp.exists(osp.join(log_path, f"photometric_d_model_{GS_BACKEND.lower()}_o.pth")):
        d_model = DynSCFGaussian.load_from_ckpt(
            torch.load(
                osp.join(log_path, f"photometric_d_model_{GS_BACKEND.lower()}_o.pth")
            ),
            device=device,
        ).to(device)
    else:
    ...
    d_model.is_opa_control = getattr(fit_cfg, "is_opa_control", True)
    photo_solver.cfg = fit_cfg
    photo_solver.photometric_fit(
        s2d=s2d,
        ...
        optimizer_cfg=OptimCFG(
            lr_cam_f=0.0,
            lr_cam_q=0.00003 if not getattr(fit_cfg, "init_gt_camera", False) else 0,
            lr_cam_t=0.00003 if not getattr(fit_cfg, "init_gt_camera", False) else 0,
            # lr_cam_q=0.00003,
            # lr_cam_t=0.00003,
            # gs
        ...
        just_dyn_region=getattr(fit_cfg, "photo_dyn_node_just_dyn_region", True),
        # Dyn clean
        ...
        train_s_model=getattr(fit_cfg, "photo_train_s_model", True),
        photo_train_opt=getattr(fit_cfg, "photo_train_opt", False),
        is_blur=getattr(fit_cfg, "is_blur", True),
        is_exposure=getattr(fit_cfg, "is_exposure", True),
        TEST_TRAINING_TIME_MODE=TEST_TRAINING_TIME_MODE,
    )
    return
...
if __name__ == "__main__":
    ...
    TEST_TRAINING_TIME_MODE = getattr(cfg, "test_training_time_mode", False)
    output_folder = cfg['data']['output'] + '/' + cfg['scene']
    start_time = strftime("%Y-%m-%d %H:%M:%S", gmtime())
    start_info = "-"*30+Fore.LIGHTRED_EX+\
                 f"\nStart Reconstruct at {start_time},\n"+Style.RESET_ALL+ \
    ...
    args.ws = os.path.join(cfg["data"]["output"] + "/" + cfg["scene"], args.ws)

    logdir = setup_recon_ws(args.ws, fit_cfg=cfg, name=getattr(cfg, "logdir_name", None))
    ...
    if not TEST_TRAINING_TIME_MODE:
        get_all_nodes(args.ws, logdir, cfg)
    ...
    s_model.to(device)
    s_model.eval()
    print(f"s_model gaussians points num: {s_model._xyz.shape[0]}")
    print(f"d_model gaussians points num: {d_model._xyz.shape[0]}")
