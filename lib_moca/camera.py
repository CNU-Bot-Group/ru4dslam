...
import logging
from src.utils.pose_utils import SE3_exp 
...
class MonocularCameras(nn.Module):
    def __init__(
        ...
        self.exposures_a = nn.Parameter(torch.zeros((self.T, 1)), requires_grad=False)
        self.exposures_b = nn.Parameter(torch.zeros((self.T, 1)), requires_grad=False)

        if T_kf is None:
            T_kf = self.T
        self.uncer_weight = torch.nn.Parameter(torch.empty((T_kf, default_H, default_W)), requires_grad=False)
        self.motion_masks = torch.nn.Parameter(torch.empty((T_kf, default_H, default_W)), requires_grad=False)
        
        self.scale_nw = torch.nn.Parameter(torch.tensor(0.0), requires_grad=False)

        self.opt_rot_start = nn.Parameter(torch.tensor([1.0, 0.0, 0.0, 0.0]).unsqueeze(0).repeat(self.T, 1), requires_grad=True)
        self.opt_trans_start = nn.Parameter(torch.tensor([0.0, 0.0, 0.0]).unsqueeze(0).repeat(self.T, 1), requires_grad=True)
        self.opt_rot_end = nn.Parameter(torch.tensor([1.0, 0.0, 0.0, 0.0]).unsqueeze(0).repeat(self.T, 1), requires_grad=True)
        self.opt_trans_end = nn.Parameter(torch.tensor([0.0, 0.0, 0.0]).unsqueeze(0).repeat(self.T, 1), requires_grad=True)
    ...
    def load_from_ckpt(cls, ckpt):
        ...
        if "_rel_focal" not in ckpt.keys():
            ...
            del ckpt["rel_focal"]
        T = len(ckpt["q_wc"])
        T_kf = len(ckpt["motion_masks"])
        cams = cls(n_time_steps=T, default_H=H, default_W=W, delta_flag=delta_flag, T_kf=T_kf)
    ...
    def __get_init_fc__(self, fovxfovycxcy, KHW):
        if KHW is None:
            ...
            rel_focal_x = 1.0 / np.tan(np.deg2rad(fovxfovycxcy[0]) / 2.0)  # fovx = 2*atan(1/fx) = 2 * fx/h
    ...
    def T_wc_start(self, ind):
        ...
        if self.delta_flag:
            ...
            R = quaternion_to_matrix(F.normalize(self.q_wc_start[ind : ind + 1], dim=-1))[0]
            t = self.t_wc_start[ind]
            T = torch.eye(4).to(R)
            T[:3, :3] = R
            T[:3, 3] = t
            return T.float()
    
    def T_wc_end(self, ind):
        # * core function
        assert ind >= 0 and ind < self.T, f"Invalid index {ind} for T={self.T}"
        if self.delta_flag:
            T_list = self.forward_T(ind)
            ...
            R = quaternion_to_matrix(F.normalize(self.q_wc_end[ind : ind + 1], dim=-1))[0]
            t = self.t_wc_end[ind]
            T = torch.eye(4).to(R)
            T[:3, :3] = R
            T[:3, 3] = t
    ...
    def T_cw_start(self, ind):
        T_wc = self.T_wc_start(ind)
        return torch.linalg.inv(T_wc)

    def T_cw_end(self, ind):
        T_wc = self.T_wc_end(ind)
        return torch.linalg.inv(T_wc)
    ...
    def update_poses(self, ind):
        tau_start = torch.cat([self.cam_trans_start_delta, self.cam_rot_start_delta], axis=0)

        T_w2c = self.T_cw_start(ind)

        new_w2c = SE3_exp(tau_start) @ T_w2c

        param_cam_q, param_cam_t = self.__get_init_qt__(new_w2c.unsqueeze(0).inverse())

        with torch.no_grad():  # 使用 no_grad() 确保不会影响计算图
            self.q_wc_start[ind] = param_cam_q[0]
            self.t_wc_start[ind] = param_cam_t[0]
        
        tau_end = torch.cat([self.cam_trans_end_delta, self.cam_rot_end_delta], axis=0)

        T_w2c = self.T_cw_end(ind)

        new_w2c = SE3_exp(tau_end) @ T_w2c
        param_cam_q, param_cam_t = self.__get_init_qt__(new_w2c.unsqueeze(0).inverse())

        with torch.no_grad():  # 使用 no_grad() 确保不会影响计算图
            self.q_wc_end[ind] = param_cam_q[0]
            self.t_wc_end[ind] = param_cam_t[0]


        self.cam_rot_start_delta.data.fill_(0)
        self.cam_trans_start_delta.data.fill_(0)
        self.cam_rot_end_delta.data.fill_(0)
        self.cam_trans_end_delta.data.fill_(0)
