...
def compute_static_ba(
    ...
    for step in tqdm(range(total_steps)):
        if step == switch_to_ind_step:
            ...
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                ...
                eta_min=max(min(lr_cam_q, lr_cam_t), 0.001) / 10.0,
            )
        ...
        with torch.no_grad():
            ...
            if not TEST_TRAINING_TIME_MODE and viz_flag:
    ...
    if not TEST_TRAINING_TIME_MODE and total_steps > 0:
    ...
    if not TEST_TRAINING_TIME_MODE and save_more_flag:
        np.savetxt(
