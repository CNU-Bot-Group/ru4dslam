...
def moca_solve(
    ...
    assert static_id_mode in ["raft", "track", "motion"], f"{static_id_mode} not in [raft, track, motion]"
    s2d.to(device)
    ...
    if static_id_mode == "raft":
        ...
        motion_mask_list = torch.Tensor(np.stack(motion_mask_list)) # T,H,W
        s2d.register_gradfree_buffer("motion_mask_list", motion_mask_list)

        with torch.no_grad():
            epierr_list = query_buffers_by_track(s2d.motion_mask_list.clone()[..., None], track, track_mask)
            epierr_list = epierr_list.squeeze(-1).cpu()
    ...
    compute_static_ba(
        ...
        viz_video_rgb=(
            ...
            if viz_valid_ba_points and not TEST_TRAINING_TIME_MODE
            else None
        ),
