    ...
    if K is not None:
        ...
        if K.ndim == 2:
            ...
            K = K[None]  # 1,T,3,3
             ...
             assert (
                K.shape[0] == 1, K.shape[1] == len(dep) and K.shape[2] == 3 and K.shape[3] == 3
            ), "K should have size: 1,T,3,3"
        else:
...
def spatracker_process_folder(
    ...
    for round in range(num_slice):
        ...
        # for t in range(T):
        #     queries_t = queries[:, queries[0, :, 0] == t].clone()
        #     queries_t[:, :, 0] = t - max(t-10, 0)
        #     video_pt_t = video_pt[:, max(t-10, 0): min(t+11, T)].clone()
        #     dep_pt_t = dep_pt[max(t-10, 0): min(t+11, T)].clone()
        #     _tracks_t, _visibility_t = infer_spa_tracker(
        #         video_pt_t, dep_pt_t, queries_t, model, K=K
        #     )  # T,N,3; T,N
        #     _tracks = torch.zeros((T, queries_t.shape[1], 3))
        #     _visibility = torch.zeros((T, queries_t.shape[1]))
        #     _tracks[max(t-10, 0): min(t+11, T)] = _tracks_t
        #     _visibility[max(t-10, 0): min(t+11, T)] = _visibility_t
        #     tracks.append(_tracks)
        #     visibility.append(_visibility)  
