...
class Saved2D(nn.Module):
    ...
    def load_dep(
        ...
        if cfg is not None:
            ...
            for frame in t:
                frame = int(frame)
                dep_dir = f'{input_folder}/depth/depth_{frame:05d}.png'
                dep = cv2.imread(dep_dir, cv2.IMREAD_UNCHANGED)
                dep = dep/1000.0
                dep = cv2.resize(dep, (W_out_with_edge, H_out_with_edge), interpolation=cv2.INTER_LINEAR)
                dep_list.append(dep)

            dep = np.stack(dep_list)  # T,H,W
    ...
    def register_2d_identification(self, static_2d_mask, dynamic_2d_mask):
        ...
        # plt.imshow(self.sta_mask[84].detach().cpu().float().numpy(), cmap="jet") # , vmin=0, vmax=1
        # plt.axis('off')
        # plt.colorbar()
        # plt.tight_layout()
        # plt.savefig(f"test_mask.png")
        # plt.close() 
