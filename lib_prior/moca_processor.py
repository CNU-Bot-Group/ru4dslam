...
class MoCaPrep:
    ...
    def create_workspace(
        self, save_dir, t_list, fn_list, img_list, imgdirname="images", TEST_TRAINING_TIME_MODE=False
    ):
        # create workspace
    ...
    def compute_depth(
        ...
        if hasattr(self, "depth_model"):
            ...
            logging.warning(f"Depth model not loaded")
        ...
        if enhance_depth_boudary_th > 0:
            ...
            original_dep = self.load_dep_list(save_dir, npz_path=depth_npz_path)
    ...
    def compute_flow(
        self, save_dir, fn_list, img_list, step_list=[1], epi_num_threads=16, TEST_TRAINING_TIME_MODE=False
    ):
        start_t = time.time()
        ...
        self.flow_process_func(
            self.flow_model, img_list, fn_list, raft_save_dir, step_list=step_list, TEST_TRAINING_TIME_MODE=TEST_TRAINING_TIME_MODE
        )
        ...
        analyze_epi(save_dir, num_threads=epi_num_threads, step_list=step_list, TEST_TRAINING_TIME_MODE=TEST_TRAINING_TIME_MODE)
        return
    ...
    def process(
        ...
        ########################################################################
        # * 0. Init
        self.create_workspace(save_dir, t_list, img_name_list, img_list, TEST_TRAINING_TIME_MODE=TEST_TRAINING_TIME_MODE)
        # ########################################################################
        # * 2. Depth
        self.compute_depth(
            save_dir,
        ...
        if compute_tap:
            ...
            dep_list = self.load_dep_list(save_dir, dep_name, depth_npz_path)
    ...
    def load_dep_list(self, ws, name=None, npz_path=None):
        if npz_path is None:
            if name is None:
