...
class Embedder(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.periodic_fns = kwargs['periodic_fns']
        
        # 计算输出维度
        self.out_dim = self.input_dims if self.include_input else 0
        self.out_dim += len(self.periodic_fns) * kwargs['num_freqs'] * self.input_dims
        
        # 创建频率带
        max_freq = kwargs['max_freq_log2']
        num_freqs = kwargs['num_freqs']
        if kwargs['log_sampling']:
            self.register_buffer('freq_bands', 2. ** torch.linspace(0., max_freq, steps=num_freqs))
        else:
            self.register_buffer('freq_bands', torch.linspace(2. ** 0., 2. ** max_freq, steps=num_freqs))
