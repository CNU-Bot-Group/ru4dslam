...
def SplineN_linear(start_pose, end_pose, poses_number, NUM, device=None):
    ...
    q_tau_0 = q_to_Q_parallel(q_to_q_conj_parallel(q_start)) @ q_end[..., None] # 相对旋转四元数 T_start^{−1}·T_end
    r = pose_time[..., None] * log_q2r_parallel(q_tau_0.squeeze(-1)) # 随时间的相对旋转so3向量 t/{\tau}*log(T_start^{−1}·T_end)
    q_t_0 = exp_r2q_parallel(r) # 随时间的相对旋转so3向量的对应旋转四元数 exp(t/{\tau}*log(T_start^{−1}·T_end))
    q_t = q_to_Q_parallel(q_start) @ q_t_0[..., None] # T_start·exp(t/{\tau}*log(T_start^{−1}·T_end))
...
def SplineN_linear_qt(start_pose, end_pose, poses_number, device=None):
    pose_time = torch.arange(poses_number, device=device).float() / (poses_number - 1)
    pose_time = pose_time.to(device)
    ...
    q_start, t_start = start_pose[:4], start_pose[4:]
    q_end, t_end = end_pose[:4], end_pose[4:]
    # sample t_vector
    q_start = q_start.unsqueeze(0)
    q_end = q_end.unsqueeze(0)
    t_start = t_start.unsqueeze(0)
    t_end = t_end.unsqueeze(0)
    pose_time = pose_time.unsqueeze(0)
    t_t = (1 - pose_time)[..., None] * t_start + pose_time[..., None] * t_end
    ...
    # poses = pose_spline.reshape([-1, 3, 4])
    q_t = q_t.squeeze(0).squeeze(-1)
    t_t = t_t.squeeze(0)
    qt = torch.cat([q_t, t_t], -1)
    return qt
