...
class RGB_NoPose(BaseDataset):
    def __init__(self, cfg, device='cuda:0'
        ...
        for i in range(len(self.color_paths)):
            color_path = self.color_paths[i]
            img = cv2.imread(color_path)
            if img.shape[0] == self.H_out_with_edge and img.shape[1] == self.W_out_with_edge:
                break
            if img is None:
                raise FileNotFoundError(f'Image {color_path} not found')
            resized_img = cv2.resize(img, (self.W_out_with_edge, self.H_out_with_edge), 
                                interpolation=cv2.INTER_LINEAR)
            cv2.imwrite(color_path, resized_img)
        self.depth_paths = None
        self.depth_paths = sorted(
